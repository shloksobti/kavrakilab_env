...
then
    kavrakilab-install-debug "geany config not existing"
    mkdir -p ~/.config/geany
    cp --verbose $TUE_DIR/installer/targets/matthijs/geany.conf  ~/.config/geany/geany.conf
else
    kavrakilab-install-debug "geany config does exists, so not copying matthijs config"
fi
...
do
        option=${item%=*}
        value=${item#*=}
        kavrakilab-install-debug "git config --global --replace-all $option $value"
